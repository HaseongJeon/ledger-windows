...
## 창을 닫아도 앱은 계속 실행됩니다 (트레이)

창을 닫아도 앱이 종료되지 않고 시스템 트레이에 남아 계속 실행됩니다. 이래야 다른 사람이
매출/지출을 새로 등록했을 때 Supabase Realtime 이 그 소식을 받아 알림을 띄울 수 있습니다
(안드로이드/데스크톱 브라우저는 FCM 푸시를 쓰지만, Electron 은 관리가 끊긴 `firebase-electron`
패키지 대신 트레이 상주 + 기본 `Notification` API 방식을 씁니다). 트레이 아이콘을 눌러 다시
창을 열 수 있고, 완전히 끄려면 트레이 메뉴의 **종료** 를 눌러야 합니다. 트레이 메뉴로 완전히
종료하면 그동안은 알림을 받지 못하지만, 다시 실행하면 최신 데이터가 바로 보이므로 데이터가
누락되지는 않습니다.
